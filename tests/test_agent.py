...
def test_workspace_reader_truncation_notice(tmp_path: Path) -> None:
    content = "A" * 100
    (tmp_path / "long.txt").write_text(content, encoding="utf-8")
    agent = make_agent(tmp_path)

    result = agent.reader.run("long.txt", max_chars=10)

    assert result.ok is True
    assert "[TRUNCATED: Only the first 10 characters are shown]" in result.output
