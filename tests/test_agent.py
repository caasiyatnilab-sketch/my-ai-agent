...
def test_workspace_reader_truncation_warning(tmp_path: Path) -> None:
    content = "A" * 10
    (tmp_path / "long.txt").write_text(content, encoding="utf-8")
    agent = make_agent(tmp_path)

    # Test with small max_chars to trigger truncation
    result = agent.reader.run("long.txt", max_chars=5)

    assert result.ok is True
    assert "AAAAA" in result.output
    assert "[TRUNCATED: Only the first 5 characters are shown]" in result.output
