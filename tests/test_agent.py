...
def test_memory_load_limit_with_large_file(tmp_path: Path) -> None:
    from my_ai_agent.memory import JsonlMemory, Message

    path = tmp_path / "large_memory.jsonl"
    ...
    assert messages[0].content == "msg 95"
    assert messages[4].content == "msg 99"
