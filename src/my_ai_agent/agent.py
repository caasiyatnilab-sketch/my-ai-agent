...
class Agent:
    ...
    def ask(self, prompt: str, *, use_memory: bool = True) -> AgentResult:
        ...
        for result in tool_results:
            # Use "user" role for tool results to ensure compatibility with
            # standard OpenAI-style providers that require tool_call_id for "tool" role.
            self.memory.append(Message("user", f"Tool output ({result.name}): {result.output}"))
