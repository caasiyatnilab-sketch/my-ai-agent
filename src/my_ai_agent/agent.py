...
class Agent:
    ...
    def ask(self, prompt: str, *, use_memory: bool = True) -> AgentResult:
        ...
        for result in tool_results:
            # Persistent memory uses "user" role for tool results to remain compatible with
            # standard OpenAI-style completions without complex tool_call_id management.
            self.memory.append(Message("user", f"Tool {result.name} output: {result.output}"))
