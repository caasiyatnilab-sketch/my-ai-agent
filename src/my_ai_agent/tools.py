...
class Calculator:
    ...
    def _eval(self, node: ast.AST) -> int | float:
        ...
        if isinstance(node, ast.BinOp) and type(node.op) in self._binary_ops:
            ...
            # Security: limit resource exhaustion BEFORE calculation
            if isinstance(node.op, ast.Pow):
                if right > 1000:
                    raise ValueError("exponent too large")
                if left != 0 and abs(right) * math.log10(abs(left)) > 100:
                    raise ValueError("result too large")
            elif isinstance(node.op, ast.Mult):
                if left != 0 and right != 0 and (math.log10(abs(left)) + math.log10(abs(right))) > 100:
                    raise ValueError("result too large")
    ...
    def run(self, relative_path: str, max_chars: int = 8_000) -> ToolResult:
        try:
            ...
            with target.open("r", encoding="utf-8") as f:
                return ToolResult("read_file", True, f.read(max_chars))
        except ValueError:
