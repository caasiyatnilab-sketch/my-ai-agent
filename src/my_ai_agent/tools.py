    ...
    def _eval(self, node: ast.AST) -> int | float:
        ...
        if isinstance(node, ast.BinOp) and type(node.op) in self._binary_ops:
            ...
            if isinstance(node.op, ast.Pow):
                ...
                # Rough check for result size to prevent nested power DoS
                # For integers, check if result would be unreasonably large
                if (
                    isinstance(left, int)
                    and isinstance(right, int)
                    and right > 0
                    and left.bit_length() * right > 50_000
                ):
                    raise ValueError("Calculation result too large")
