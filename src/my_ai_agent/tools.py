...
class Calculator:
    ...
    def _eval(self, node: ast.AST) -> int | float:
        ...
        if isinstance(node, ast.BinOp) and type(node.op) in self._binary_ops:
            ...
            # Mitigate DoS risks from resource-intensive calculations.
            ...
            # Bound the magnitude of intermediate and final results.
            if isinstance(result, int) and result.bit_length() > 10000:
                raise ValueError("Result too large")
            if isinstance(result, float) and abs(result) > 1e100:
                raise ValueError("Result too large")
