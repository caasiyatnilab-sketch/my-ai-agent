...
class Calculator:
    ...
    def _eval(self, node: ast.AST) -> int | float:
        ...
        if isinstance(node, ast.BinOp) and type(node.op) in self._binary_ops:
            ...
            # DoS protection: limit resource-intensive operations
            if isinstance(node.op, ast.Pow):
                if right > 1000:  # Prevent huge numbers from power ops
                    raise ValueError("exponent too large")
            if isinstance(node.op, ast.Mult):
                if isinstance(left, int) and isinstance(right, int):
                    # Prevent quadratic growth from large integer multiplications
                    if left.bit_length() + right.bit_length() > 10000:
                        raise ValueError("multiplication result too large")

            result = self._binary_ops[type(node.op)](left, right)

            # Prevent memory exhaustion when converting extremely large results to strings.
...
class WorkspaceReader:
    ...
    def run(self, relative_path: str, max_chars: int = 8_000) -> ToolResult:
        try:
            ...
            # DoS protection: only read the requested amount from the file
